...
const Register = (props) => {
    ...
    const onChange = (value, type) => {
        ...
        if (hasLeaved[type].get) { validSetters[type](formValidation(value, type)); }
    }
    ...
    const canRegister = emailIsValid && usernameIsValid && passwordIsValid && passwordConfirmed;
    ...
    return (
        <React.Fragment>
            <TextInput placeholder="E-mail" invalid={!emailIsValid & hasLeavedEmail} 
                onChange={e => onChange(e.target.value, 'email')} 
                onBlur={e => onLeave(e.target.value, 'email')} />
            <TextInput placeholder="Username" invalid={!usernameIsValid & hasLeavedUsername} 
            ...
            <TextInput placeholder="Confirm password" type="password" invalid={!passwordConfirmed & hasLeavedPassword} 
                onChange={e => setPasswordConfirmed(e.target.value == password) } />
            <Button color='#50514F' disabled={!canRegister} onClick={onRegister}>Register</Button>
            <SocialLogin />
        </React.Fragment>
    );
}
